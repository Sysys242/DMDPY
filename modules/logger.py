...
class Logger:
    ...
    def print_banner(self, status='', clear:bool=True, delay:float=0.01):
        if clear:
            self.clear()
 ...
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  ________    | || | ____    ____ | || |  ________    | || |   ______     | || |  ____  ____  | |
| | |_   ___ `.  | || ||_   \  /   _|| || | |_   ___ `.  | || |  |_   __ \   | || | |_  _||_  _| | |
| |   | |   `. \ | || |  |   \/   |  | || |   | |   `. \ | || |    | |__) |  | || |   \ \  / /   | |
| |   | |    | | | || |  | |\  /| |  | || |   | |    | | | || |    |  ___/   | || |    \ \/ /    | |
| |  _| |___.' / | || | _| |_\/_| |_ | || |  _| |___.' / | || |   _| |_      | || |    _|  |_    | |
| | |________.'  | || ||_____||_____|| || | |________.'  | || |  |_____|     | || |   |______|   | |
| |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------' """)))
        self.delay_print(self.center(status), delay)

    def delay_print(self, text:str, delay:float=0.01):
        with self.lock:
            Write.Print(text +"\n",Colors.green_to_yellow,delay)
    
    def delay_input(self, text:str, delay:float=0.01):
        with self.lock:
            return Write.Input(text,Colors.green_to_yellow,delay)
    
    def error(self, text:str):
        with self.lock:
            print(f"{Colors.gray}~{Colors.light_red}ERROR{Colors.gray}~ {Colors.light_gray}{text}")
    
    def info(self, text:str):
        with self.lock:
            print(f"{Colors.gray}~{Colors.light_blue}INFO{Colors.gray}~ {Colors.light_gray}{text}")
    
    def success(self, text:str):
        with self.lock:
            print(f"{Colors.gray}~{Colors.light_green}SUCCESS{Colors.gray}~ {Colors.light_gray}{text}")
    
    def debug(self, text:str):
        if self._debug:
            with self.lock:
                print(f"{Colors.gray}~{Colors.cyan}DEBUG{Colors.gray}~ {Colors.light_gray}{text}")